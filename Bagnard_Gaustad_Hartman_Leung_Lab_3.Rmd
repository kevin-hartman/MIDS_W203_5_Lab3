...
Given that our analysis of model 2 showed that taxpc was not statistically significant, we will not consider it in our model.
...
$$log(crmrate) = \beta_0 + \beta_1log(crimjusteff) + \beta_2log(polpc) + \beta_3log(scaledWages) + \beta_4regcode + \beta_5log(pctmin80) + \beta_6density +u$$

### Model 3 Linear Model
```{r, include=TRUE, comment=NA, warning=FALSE, echo=TRUE}
model3_initial<-lm(logcrmrte ~ logcrimJustEff + logScaledWages +  regcode + logpolpc + logpctmin80 + density, data = dfCrime)
...
$$log(crmrate) = \beta_0 + \beta_1log(crimjusteff) + \beta_2log(polpc) + \beta_3log(scaledWages) + \beta_4log(pctmin80) + \beta_5density +u$$
```{r, include=TRUE, comment=NA, warning=FALSE, echo=TRUE}
model3<-lm(logcrmrte ~ logcrimJustEff + logScaledWages +  logpolpc + logpctmin80 + density, data = dfCrime)
model3
...
* scaledWages: If we increase wages by 1 percent, we expect the crime rate to increase by 0.95%
