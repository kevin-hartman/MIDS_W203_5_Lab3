...
Our base hypothesis is that crime can be fundamentally explained by three factors: the effectiveness of the criminal justice system, economic conditions, and regional differences..
...
We will again modify this variable with a log transformation for better interpretation. This allows us to interpret percent changes in wage which is more consistent across a range of wages. The result is a relatively normal distribution as seen in the below histogram.


```{r, include=TRUE, comment=NA, warning=FALSE, echo=TRUE}
dfCrime$logScaledWages <- log(dfCrime$scaledWages)
hist(dfCrime$logScaledWages)
ggplot(data=dfCrime, aes(sample= logScaledWages)) + stat_qq() + stat_qq_line() +
  ggtitle("QQ Plot of log scaled wages")
```

### Model 1 Linear Model

$$ log(Crime Rate) = B_0 + B_1log(Unweighted Average Wage) + B_2log(Criminal Justice Effectiveness)$$
...
* **MLR3** No perfect multicollinearity: None of the variables chosen for the model are constant or perfectly collinear as the economy and criminal justice effectiveness are independent.  Our VIF value shows very little colinearity.
```{r}
...
```{r}
cov(resid(mod1),dfCrime$logScaledWages)
cov(resid(mod1),dfCrime$logcrimJustEff)
mean(resid(mod1))
```
* **MLR4** The zero conditional mean assumption is well supported when viewing the Residuals vs fitted plot.  The spline fit is nearly flat and centered at 0.
```{r, include=TRUE, comment=NA, warning=FALSE, echo=TRUE}
plot(mod1, which=1)
```
* **MLR5** There does appear to be heteroskedacity in the 'lips' appearance of the Residuals vs fitted plot.  This is acknowledged and can be accounted for by using the heteroskedastic robust standard errors.  This is seen in the coeftest.
```{r}
coeftest(mod1, vcov=vcovHC) #coefficients with heteroskedastic consistent standard errors
```
* **MLR6** The final assumption of linear regression is that the errors are normall distributed.  This appears to hold for the bulk of the residuals with some skewness in the tails.  This is also reflected in the significant return on the shapiro test.  The model should not be used when predicting crime rate for counties with extreme criminal justice effectiveness or wages.  
```{r, include=TRUE, comment=NA, warning=FALSE, echo=TRUE}
shapiro.test(mod1$residuals) # test for normality
plot(mod1, which=2) # QQ plot for residuals
```

The model gives estimates and standard errors that are heteroskedastic consistent.  The coefficient of the log of the unweighted_avg_wage is calculated to have a coefficient of 1.85.  This means that an increase of 1% in wages is correlated with an increase of 0.5% in crime rate.  Generally increased wages are not associated with increased crime.  This suggests that wages are correlated with a stronger omitted variable that affects crime.  In model 2, we will investigate regional variations that could explain the positive correlation of wages and crime.  TALK MORE ABOUT THIS CORRELATION >>> OMitted variable bias --> mean wage is easily affected by extreme high wages, perhaps income inequality.

```{r}
q9<-ggplot(data = dfCrime, aes(x = logScaledWages, y = logcrmrte, color = urban)) + 
      geom_point()+
  geom_smooth(method = "lm")
q9
```

Similarly, criminal justice effectiveness (convictions/crime) is given a coefficient of -0.489 which suggests that an increase of 1% increase in convictions per crime will decrease crime by nearly .5%.  This suggests that we have found a relatively strong correlation and perhaps a good influence on crime rate in a county.
...
NEED TO ADD REGION INTRODUCTION
...
$$log(crmrate) = \beta_0 + \beta_1crimjusteff + \beta_2log(polpc) + \beta_3density + \beta_4scaledWages + \beta_5taxpc + u$$

### Model 2 EDA and Data Transformations

To start, we will look at how police per capita is better 
```{r, include=TRUE, comment=NA, warning=FALSE, echo=TRUE}
...
p2 <- ggplot(dfCrime, aes(x = log(polpc), color=regcode, fill = regcode)) + 
  geom_histogram(position="identity", alpha=0.5, bins=30) +
  labs(title="Log TransformedPolice per Capita", x="Log(Police per Capita)", y="Count")
grid.arrange(p1, p2, ncol=2)
```
why WE LIKE LOG of police per capita
...
grid.arrange(p1, p2, ncol=2)
```
In tax revenue, we see that each region is approximately normal but centered on a different mean.... and we like log.  more normal for each region. log transform helps outlier,  outlier--> this could be major economic hub...
...
q9<-ggplot(data = dfCrime, aes(x = logScaledWages, y = logcrmrte, color = regcode)) + 
      geom_point()+
  geom_smooth(method = "lm")
q9a<-ggplot(data = dfCrime, aes(x = log(wfed), y = log(crmrte), color = regcode)) + 
      geom_point()+
  geom_smooth(method = "lm")

options(repr.plot.width=8, repr.plot.height=16)
grid.arrange(q9,q9a, ncol=2)
```

```{r, include=TRUE, comment=NA, warning=FALSE, echo=TRUE}

q9<-ggplot(data = dfCrime, aes(x = logtaxpc, y = logcrmrte, color = regcode)) + 
      geom_point()+
  geom_smooth(method = "lm")
q9a<-ggplot(data = dfCrime, aes(x = logpolpc, y = logcrmrte, color = regcode)) + 
      geom_point()+
  geom_smooth(method = "lm")
...
```{r, include=TRUE, comment=NA, warning=FALSE, echo=TRUE}
model2 <- lm(logcrmrte ~ logcrimJustEff + logpolpc*regcode + logtaxpc + logScaledWages, data = dfCrime)
model2
...
* **MLR5: Homoscedasticity** The above Residuals vs Fitted graph provides evidence of heteroscedasticity as right side of the chart have fewer datepoints. To provide further evidence of heteroscedasticity, we will use the White test to generate coefficients with robust with vcovHC
...
In addition, we hypothesize that the location of the county can have an impact on demographics. We can see that counties in the West have a significantly lower percentage of minorities than the other two regions. As a result, we will also test for region in our model. 
...
$$log(crmrate) = \beta_0 + \beta_1log(crimjusteff) + \beta_2log(polpc) + \beta_3density + \beta_4log(scaledWages) + \beta_5logpctmin80 + \beta_6west + \beta_7central+u$$
...
model3_initial<-lm(logcrmrte ~ logcrimJustEff + logpolpc + logScaledWages +  density + logpctmin80 + regcode, data = dfCrime)
...
$$log(crmrate) = \beta_0 + \beta_1log(crimjusteff) + \beta_2log(polpc) + \beta_3density + \beta_4log(scaledWages) + \beta_5logpctmin80 +u$$
```{r, include=TRUE, comment=NA, warning=FALSE, echo=TRUE}
model3<-lm(logcrmrte ~ logcrimJustEff + logpolpc + logScaledWages +  density + logpctmin80, data = dfCrime)
model3
...
* scaledWages: If we increase wages by 1 percent, we expect the crime rate to increase by 0.94%
* Density: If we increase density by 1 person per square mile, we expect the crime rate to increase by 8%
* Percentage of minorities: If the percentage of minorities increase by 1%, we expect the crime rate to increase by 0.24%

Negative coefficients:
* Criminal justice efficiency: If we increase the criminal justice efficiency by 1%, we expect the crime rate to decrease by 0.42%.
...
```{r, include=TRUE, comment=NA, warning=FALSE, echo=TRUE}
stargazer(mod1,model2,model3,type="text", option())
```
